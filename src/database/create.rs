...
use crate::game::controller::LostBy;
...
pub fn insert_game_into_db(game_data: &BoardMetaData) -> Result<()>{
    let con = Connection::open("chess.db")?;
    ...
    let generated_id = con.last_insert_rowid();

    let mut stmt = con.prepare(
        "INSERT INTO moves (game_id, uci, san, eval_score) VALUES (?1, ?2, ?3, ?4)",
    )?;

    for ply in &game_data.move_list {
        stmt.execute(params![generated_id, &ply.uci, &ply.san, ply.evaluation])?;
    }
