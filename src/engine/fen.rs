...
impl Board {
    pub fn set_fen(&mut self, fen: String){
        ...
        if parts.len() != 6 {
            eprintln!("{}:{} Invalid FEN: expected 6 fields, got {}", file!(), line!(), parts.len());
            return;
        }
        ...
        let halfmove_clock: u32  = match parts[4].parse() {
            Ok(v) => v,
            Err(e) => {
                eprintln!("{}:{} Invalid halfmove clock '{}': {}", file!(), line!(), parts[4], e);
                0
            }
        };
        let fullmove_number: u32 = match parts[5].parse() {
            Ok(v) => v,
            Err(e) => {
                eprintln!("{}:{} Invalid fullmove number '{}': {}", file!(), line!(), parts[5], e);
                1
            }
        };
        ...
        if fen_files.len() != 8 {
            eprintln!("{}:{} Invalid FEN board rows: expected 8, got {}", file!(), line!(), fen_files.len());
            return;
        }
        ...
        let en_passant_target = if en_passant_targets == "-" {
            ...
            if bytes.len() != 2 || bytes[0] < b'a' || bytes[0] > b'h' || bytes[1] < b'1' || bytes[1] > b'8' {
                eprintln!("{}:{} Invalid en-passant square '{}'", file!(), line!(), en_passant_targets);
                None
            } else {
        ...
        // Apply to self
        self.squares = board;
        self.turn = if to_move == "w" { PieceColor::White } else { PieceColor::Black };
